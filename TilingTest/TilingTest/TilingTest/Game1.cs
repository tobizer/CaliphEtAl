...
namespace TilingTest
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (ks.IsKeyDown(Keys.Left) && reset == true)
            {
                Location.X = MathHelper.Clamp(Location.X - 1, 0,
                    (myMap.MapWidth - squaresAcross) * Tile.TileStepX);
                reset = false;
            }

            if (ks.IsKeyDown(Keys.Right) && reset == true)
            {
                Location.X = MathHelper.Clamp(Location.X + 1, 0,
                    (myMap.MapWidth - squaresAcross) * Tile.TileStepX);
                reset = false;            
            ...
            if (ks.IsKeyDown(Keys.Up) && reset == true)
            {
                Location.Y = MathHelper.Clamp(Location.Y - 1, 0,
                    (myMap.MapHeight - squaresDown) * Tile.TileStepY);
                reset = false;
            }

            if (ks.IsKeyDown(Keys.Down) && reset == true)
            {
                Location.Y = MathHelper.Clamp(Location.Y + 1, 0,
                    (myMap.MapHeight - squaresDown) * Tile.TileStepY);
                reset = false;
